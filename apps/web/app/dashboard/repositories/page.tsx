    ...
    <div className="p-4 pb-12 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/35">{repositories?.length || 0} connected</p>
        </div>
      ...
      {isLoading ? (
        <div className="space-y-2">{[1,2,3,4].map((i) => <div key={i} className="h-20 rounded-2xl shimmer" />)}</div>
      ) : filtered.length === 0 ? (
        <div className="arch-card p-12 text-center">
          ...
          <p className="text-xs text-white/25 mt-1">
            {search ? "Try a different search term." : "Connect your first GitHub repository to get started."}
          </p>
          {!search && (
            <button onClick={() => setNewRepoOpen(true)}
