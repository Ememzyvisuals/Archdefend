...
import {
  ...
  NodeTypes, Handle, Position, MarkerType, getRectOfNodes, getTransformForBounds
} from "@xyflow/react";
import "@xyflow/react/dist/style.css";
import { motion, AnimatePresence } from "framer-motion";
import {
  ArrowLeft, Search, Filter, X, GitBranch,
  Code2, Box, Zap, Database, Globe, Info,
...
function ArchNode({ data, selected }: any) {
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Top} style={{ background: cfg.color, width: 6, height: 6, border: "none", opacity: 0.7 }} />

      <div className="flex items-center gap-2 mb-1">
        <div style={{ background: cfg.bg, borderRadius: 5, padding: 3, display: "flex" }}>
          <Icon size={11} style={{ color: cfg.color }} />
        </div>
        <span style={{ fontSize: 10, color: cfg.color, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: "0.06em", opacity: 0.8 }}>
          {data.type}
        </span>
      </div>

      <p style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.92)", marginBottom: 2, wordBreak: "break-word" }}>
        {data.name}
      </p>
      ...
      {data.file_path && (
        <p style={{ fontSize: 9, color: "rgba(255,255,255,0.2)", marginTop: 2, fontFamily: "monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {data.file_path}
        </p>
      )}

      <Handle type="source" position={Position.Bottom} style={{ background: cfg.color, width: 6, height: 6, border: "none", opacity: 0.7 }} />
    </div>
  );
}
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        style={{ background: "#0f0f14", border: "1px solid rgba(255,255,255,0.1)", backdropFilter: "blur(12px)" }}
      >
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-semibold text-white/60 uppercase tracking-wider">Node Inspector</span>
        ...
        <div className="space-y-1.5">
          {[
            ...
            ["Lines", node.data.line_start ? `${node.data.line_start}–${node.data.line_end || node.data.line_start}` : null],
          ].filter(([, v]) => v).map(([k, v]) => (
            <div key={String(k)}>
...
function FilterPanel({ activeFilters, onToggle, counts }: any) {
  return (
...
export default function ArchitectureMapPage() {
  ...
  useEffect(() => {
    ...
    const filtered = graphData.nodes.filter((n: any) => {
      const matchesSearch = !search || n.data.name.toLowerCase().includes(search.toLowerCase()) ||
        n.data.file_path?.toLowerCase().includes(search.toLowerCase());
      const matchesFilter = activeFilters.includes(n.data.type);
    ...
    setNodes(filtered);
    setEdges(filteredEdges.map((e: any) => ({
      ...e,
  ...
  return (
    <div className="h-screen flex flex-col bg-[#080810]">
      ...
      <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0"
        style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(10,10,18,0.95)", backdropFilter: "blur(12px)" }}>
      ...
      {showFilters && (
        <div className="px-4 py-2 flex-shrink-0"
          ...
          <FilterPanel activeFilters={activeFilters} onToggle={toggleFilter} counts={typeCounts} />
        </div>
      )}
      ...
      <div className="flex-1 relative">
        ...
        >
          ...
          <MiniMap
            ...
            nodeColor={(node: any) => NODE_CONFIG[node.data?.type]?.color || "#6366f1"}
          ...
          <Panel position="bottom-left">
            <div className="rounded-xl p-3 mb-2"
              style={{ background: "rgba(10,10,18,0.95)", border: "1px solid rgba(255,255,255,0.08)", backdropFilter: "blur(8px)" }}>
