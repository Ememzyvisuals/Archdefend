...
import {
  ...
  Settings, Menu, X, LogOut, Crown, ChevronRight,
  Shield
} from "lucide-react";
...
import { workspacesApi, notificationsApi } from "@/lib/api";
...
function SidebarContent({ pathname, plan, user, unreadCount, onClose, onLogout, isActive }: any) {
  return (
    <div className="flex flex-col h-full p-4">
      {/* Logo + close */}
      <div className="flex items-center justify-between mb-8">
        <Logo size={26} />
        <button onClick={onClose} className="p-1.5 text-white/30 hover:text-white/70 transition-colors lg:hidden">
          <X size={16} />
        </button>
      </div>
          ...
          return (
            <Link key={item.href} href={item.href} onClick={onClose}>
              <div className={`nav-item ${active ? "active" : ""}`}>
                ...
                {item.label === "Security" && (
                  <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded-full bg-red-500/15 text-red-400 border border-red-500/20">
                    !
                  </span>
                )}
              </div>
      ...
      <div className="flex items-center gap-3 pt-3"
        ...
        {user?.avatar_url ? (
          <img src={user.avatar_url} alt={user.name} className="w-8 h-8 rounded-full border border-white/10 flex-shrink-0" />
        ) : (
        ...
        <button onClick={onLogout} className="p-1.5 text-white/25 hover:text-white/60 transition-colors">
          <LogOut size={14} />
        </button>
      </div>
    </div>
