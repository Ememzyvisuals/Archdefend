...
function UsageMeter({ label, icon: Icon, used, limit, color = "#6366f1" }: {
  label: string; icon: any; used: number; limit: number; color?: string;
}) {
...
const PLAN_FEATURES: Record<string, string[]> = {
  starter:    ["3 repositories", "30 analyses/month", "30 reports/month", "All report types", "Security scanning", "PDF & Markdown export"],
  pro:        ["20 repositories", "200 analyses/month", "100 reports/month", "Team workspaces", "PPTX export", "Drift detection", "Priority support"],
  enterprise: ["Unlimited repositories", "Unlimited analyses", "Unlimited reports", "Unlimited members", "RBAC permissions", "SLA support"],
};
  ...
  return (
    <div className="p-4 pb-12 space-y-5">
      ...
      {sub && (
        <div>
          ...
          <div className="space-y-3">
            <UsageMeter label="Repositories"      icon={GitBranch} used={1} limit={sub.repositories_limit}  color="#6366f1" />
            <UsageMeter label="Analyses run"      icon={Zap}       used={0} limit={sub.analyses_per_month}  color="#06b6d4" />
            <UsageMeter label="Reports generated" icon={FileText}  used={0} limit={sub.reports_per_month}   color="#8b5cf6" />
          </div>
        </div>
      )}
                ...
                <div className="arch-card p-5"
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/35">{PLAN_DESCS[plan.id] || ""}</p>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() => !isCurrent && paymentMutation.mutate(plan.id)}
      ...
      <div className="p-4 rounded-xl text-xs text-white/30 leading-relaxed"
        ...
        💳 Payments via NOWPayments. We accept USDT, BTC, ETH and 50+ cryptocurrencies.
